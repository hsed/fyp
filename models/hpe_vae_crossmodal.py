...
'''
    Cross-modal Deep Variational Hand Pose Estimation (Spurr 2018)
    https://arxiv.org/abs/1803.11404
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, z_dim, hand_side_invariance=False, depth=False):
        self.inplanes = 64
...
def resnet18(z_dim, hand_side_invariance, depth=False):
    """Constructs a ResNet-18 model.
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], z_dim, hand_side_invariance, depth)

    return model

def resnet34(z_dim, hand_side_invariance, depth=False):
    """Constructs a ResNet-34 model.
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3], z_dim, hand_side_invariance, depth)
...
class rgb_decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        in_conv = out_lay.view(-1, 256, 4, 4)
        out_conv = self.conv_blocks(in_conv)
        return out_conv


class joints_encoder(nn.Module):
    def __init__(self, z_dim, in_dim, hand_side_invariance=False):
        ...
        if hand_side_invariance:
            self.last_lay = nn.Linear(512 + 2, 2 * z_dim)
        else:
            self.last_lay = nn.Linear(512, 2 * z_dim)

    def forward(self, x, hand_side=None):
        lay_out = self.lin_lays(x.view(-1, self.in_size))

        if hand_side is None:
            out_lay = self.last_lay(lay_out)
        else:
            out_lay = self.last_lay(torch.cat((lay_out, hand_side), dim=1))
        return out_lay
...
## main model skeleton code
'''

Require one vae module of depth to depth --> using rbg enc/dec
another vae module of joints to joints --> using joints enc/dec
